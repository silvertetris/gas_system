...
- **완료**: `prex/` 전처리 파이프라인, `trend/` EDA 시각화 파이프라인.
- **다음 단계**: 고장 라벨링(진짜 고장 vs. transient 판정) → 피처 엔지니어링 → 모델링(KF-PINN 등).
...
- 출력 (`prex/output/`, git 미포함):
  - `trend_htr31p.parquet` (7,804,924행): PI21X/TI21Y/TI21Z/TI33P/TI-D2P/PI-D2P + segment_id
