...
# ---------------------------------------------------------------------------
# FAULT 태그 → 물리 계통 (2026-09-11 신설)
# ---------------------------------------------------------------------------
# 왜 필요한가: 태그 코드만 보면 `TALD1P` 가 수조인지 버너인지 알 수 없다. EDA 그림·표에
# 계통을 같이 찍어야 "어느 계통이 자주 고장나나"를 읽을 수 있다.
# 분류 근거: data/docs/02_KPOS_태그사전.md §1 (히터 부속 계통 문자 A/B/C/D).
#   A=연료가스(FG) 압력계통 · B=버너/연소 · D=열매체액(수조) · 3x=차단밸브
FAULT_SUBSYSTEM = {
    "TALLD1P": "수조",       "TALD1P": "수조",      "TAHD1P": "수조",   "GAD1P": "수조",
    "BAB1P":   "연소",       "GAB1P":  "연소",
    "PAHA1P":  "연료가스",   "PAHHA1P": "연료가스",
    "ZAF31P":  "차단밸브",
    "GLAB1P":  "안전(누출)",
}

# 고장예측 관점의 중요도 — 문서 04 §2 "예측 가치" 열 기준
FAULT_PRIORITY = {
    "BAB1P": "★★★", "TALLD1P": "★★★", "GAB1P": "★★★",
    "TALD1P": "★★☆", "TAHD1P": "★★☆", "GAD1P": "★★☆",
    "PAHHA1P": "★★☆", "GLAB1P": "★★☆", "ZAF31P": "★★☆", "PAHA1P": "★☆☆",
}
...
# ---------------------------------------------------------------------------
# 정비(정기점검) 시점 탐지 — 열화 분석의 구간 경계 (2026-09-11 신설)
# ---------------------------------------------------------------------------
# 왜 필요한가: 정비는 전열면을 세척해 `U·A` 를 계단식으로 **회복**시킨다(docs §9-3b 실측:
# 2021 정비 ε 0.182→0.236, 2024 정비 0.273→0.640). 전 구간을 한 줄로 회귀하면 열화(하락)와
# 정비(상승)가 상쇄돼 "열화 없음"으로 잘못 보인다(§8-3 의 오류). **정비로 끊어서 봐야 한다.**
#
# 탐지 규칙: 수조가 식고(계기 dropout 포함) 진공이 풀린 상태가 오래 이어지면 개방정비다.
# 이 규칙으로 전 구간을 훑으면 정기점검 기록 7건(2011·2013·2015·2017·2019·2021·2024)이
# **전부 잡힌다**(§10-1). 2021·2024 는 점검보고서 기재 날짜와 일치해 규칙이 검증됐다.
# → 나머지 4개 연도의 스캔 PDF 를 OCR 하지 않아도 정비 시점을 얻을 수 있다.
MAINT_BATH_TEMP_MAX = 1.0      # 수조 수온이 이 미만이면 정지로 본다 [℃] (0 = 계기 dropout)
MAINT_VACUUM_MAX = -750.0      # 진공압이 이 미만이면 스팬끝 = 정지 [mmHg]
MAINT_MIN_HOURS = 24.0         # 이 시간 이상 이어져야 정비로 인정 (짧은 정지는 운전 정지)


# ---------------------------------------------------------------------------
# 열전달 부하보정 — 직렬 저항 모델 (2026-09-11 신설, 구 m^0.8 단순식 폐기)
# ---------------------------------------------------------------------------
# 제작도서 열전달 계산서의 **내막/외막 계수**. 이게 있어야 부하보정을 제대로 한다.
#   1/U = 1/α_io + 1/α_o   →  설계점에서 가스측 저항 30%, 수조측 70%
# 가스 유량이 변하면 **가스측만** Dittus-Boelter 로 변한다(α_io ∝ m^0.8). 수조측은 그대로다.
#
# ⚠ 구 방식 `UA = UA_ref·(m/m_d)^0.8` 은 전체 저항이 가스측이라고 가정한 것이라 과보정이었다.
#   실측 오차(2026-09-11): m/m_d=0.2 에서 −51%, 0.58 에서 −25%, 1.5 에서 +27%.
#   주기마다 m_gas 가 1.87~11.64 kg/s 로 달라 **주기 간 비교 자체가 오염**됐다.
HTR_ALPHA_IO_KCAL = 2048.0   # 내막(가스측) 열전달계수 [kcal/m²·h·℃] — 제작도서 설계값
HTR_ALPHA_O_KCAL = 863.1     # 외막(수조측) 열전달계수 [kcal/m²·h·℃]
GAS_SIDE_EXPONENT = 0.8      # Dittus-Boelter: 가스측 α ∝ m^0.8 (교과서값, T3)
